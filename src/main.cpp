...
void renderDisplayAndLog(float pressureAvg, float waterTempAvg, int16_t oilTemp, int16_t maxOilTemp)
{
  ...
  if (pressureChanged)
  {
    ...
    drawFillArcMeter(mainCanvas, pressureAvg, 0.0f, MAX_OIL_PRESSURE_METER, 8.0f, COLOR_RED, "BAR", "OIL.P",
                     recordedMaxOilPressure, 0.5f, true, 0, 60);
  ...
  if (waterChanged)
  {
    ...
    drawFillArcMeter(mainCanvas, waterTempAvg, 50.0f, 110.0f, 98.0f, COLOR_RED, "Celsius", "WATER.T", recordedMaxWaterTemp,
                     5.0f, false, 160, 60);
...
void updateGauges()
{
  ...
  // 平滑化: 新しい値に徐々に近づける
  smoothWaterTemp += TEMP_DISPLAY_SMOOTHING_ALPHA * (targetWaterTemp - smoothWaterTemp);
  smoothOilTemp += TEMP_DISPLAY_SMOOTHING_ALPHA * (targetOilTemp - smoothOilTemp);
  ...
  renderDisplayAndLog(pressureAvg, smoothWaterTemp, oilTempDisplay, recordedMaxOilTempTop);
}
