...
void drawFillArcMeterStatic(M5Canvas& canvas, float minValue, float maxValue, float threshold, float tickStep,
                            const char* unit, const char* label, int x, int y);
...
void renderDisplayAndLog(float pressureAvg, float waterTempAvg, int16_t oilTemp, int16_t maxOilTemp)
{
  ...
  if (pressureChanged)
  {
    drawFillArcMeter(mainCanvas, pressureAvg, 0.0f, 10.0f, 8.0f, RED, recordedMaxOilPressure, true, 0, 60);
    displayCache.pressureAvg = pressureAvg;
  }

  if (waterChanged)
  {
    drawFillArcMeter(mainCanvas, waterTempAvg, 50.0f, 110.0f, 98.0f, RED, recordedMaxWaterTemp, false, 160, 60);
...
void setup()
{
  ...
  drawFillArcMeterStatic(mainCanvas, 0.0f, 10.0f, 8.0f, 0.5f, "BAR", "OIL.P", 0, 60);
  mainCanvas.fillRect(160, 60, 160, GAUGE_H, COLOR_BLACK);
  drawFillArcMeterStatic(mainCanvas, 50.0f, 110.0f, 98.0f, 5.0f, "Celsius", "WATER.T", 160, 60);
  mainCanvas.pushSprite(0, 0);
